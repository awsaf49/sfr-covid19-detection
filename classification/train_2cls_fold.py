...
def build_model(dim=[512,512], model_name='efficientnet_b0',weights="imagenet",compile_model=True,
    ...
    if compile_model:
        ...
        f1  = tfa.metrics.F1Score(num_classes=NUM_CLASS,average="macro",threshold=0.5)
...
if __name__ == "__main__":
    ...
    if use_ricord:
        ...
        tqdm.pandas(desc="Mapping ricord labels  ")
        extr_df['class_name']  = extr_df['class_label'].progress_apply(lambda x: label2name[x])
    ...
    for fold in range(5):           
        ...
        if use_ricord:
            ricord_paths  = ricord_df.image_path.values
            ricord_labels = ricord_df["class_label"].values.astype(np.float32)
            train_paths   = np.concatenate((train_paths, ricord_paths), axis=0)
            train_labels  = np.concatenate((train_labels, ricord_labels),axis=0)


        print("## Train image Size: (%i, %i) | batch_size %i  | model_name %s | weights %s ##\n"%
              (params["IMG_SIZES"][0][0],params["IMG_SIZES"][0][1],BATCH_SIZES[0]*REPLICAS,MODEL_NAME, PRETRAIN))
        print('## Training: %i | Validation: %i ##'%(len(train_paths), len(val_paths)))
        ...
        sv = tf.keras.callbacks.ModelCheckpoint(
            os.path.join(MODEL_DIR,'fold-%i.h5'%fold), monitor='val_auc',
