...
def build_model(dim=[512,512], model_name='efficientnet_b0',weights="imagenet",compile_model=True,
    ...
    if compile_model:
        ...
        f1  = tfa.metrics.F1Score(num_classes=NUM_CLASS,average="weighted",threshold=0.5)
        prc = tf.keras.metrics.Precision(thresholds=0.5, top_k=None)
        rcl = tf.keras.metrics.Recall(thresholds=0.5, top_k=None)

        model.compile(optimizer=opt,loss=loss,metrics=[auc, prc, rcl, f1, acc])
    return model
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cfg", type=str,default="classification/chex_cfg.json" ,help="config path")
    ...
    df = pd.read_csv(os.path.join('.','data','meta','meta_chex.csv'))
    df['image_path'] = df.Path.map(lambda x: x.replace('CheXpert-v1.0',TRAIN_PATH))
    df = df[~df[df.columns[3]].str.contains("Lateral")]
    df = df.drop(["Sex", "Age", "Frontal/Lateral", "AP/PA"], axis=1)
    df = df.replace(-1,1)
    df['patient_id'] = df.image_path.map(lambda x: x.split('/')[-3])
    df['study'] = df.image_path.map(lambda x: x.split('/')[-2])
    ...
    if DISPLAY_PLOT:
        ...
        plt.legend(loc=3)
        plt.savefig(os.path.join(MODEL_DIR,"loss_plot_chex.png"))
        #plt.show()
