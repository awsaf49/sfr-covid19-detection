...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        method='nms',merge_nms=False, sigma=0.1, img_size=512,
                        labels=(), max_det=300):
    ...
    Additional arguments for ensemble-boxes repo:
        method: 'nms' - default nms 
                'snms' - soft-nms (requires sigma)
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if method != 'nms':
            ...
            if method == 'snms':
                boxes, scores, new_labels = soft_nms([x[:, :4].cpu()],[x[:, 4].cpu()],[x[:, 5].cpu()], iou_thr=iou_thres, 
                                        sigma=sigma)
            elif method == 'wbf':
                boxes, scores, new_labels = weighted_boxes_fusion([x[:, :4]],[x[:, 4]],[x[:, 5]],
                                        iou_thr=iou_thres)
            elif method  == 'nmw':
                boxes, scores, new_labels = non_maximum_weighted([x[:, :4]],[x[:, 4]],[x[:, 5]],
                                        iou_thr=iou_thres)
            ...
            if len(boxes) > 0:
                ...
                x[:, 5] = torch.tensor(new_labels, device=x.device)
            else:
                x = torch.tensor([], device=x.device)
            x[:, :4] *= img_size
            if (max_det != -1 ) and  (x.shape[0] > max_det):  # limit detections
                x = x[:max_det]
            output[xi] = x
