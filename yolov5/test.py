...
def test(data,
    ...
    if training:  # called by train.py
        ...
        # Load model
        model = attempt_load(weights, map_location=device, ensemble_mode=ensemble_mode)  # load FP32 model
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            out = non_max_suppression(
                ...
                method=pp_method, sigma=sigma, img_size=height # assumes height = width
            )
...
if __name__ == '__main__':
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        test(opt.data,
             ...
             pp_method=opt.method,
             ensemble_mode=opt.ensemble_mode,
             sigma=opt.sigma,
             opt=opt
             )
